import {_,a7 as Bn,s as se$1,c as cB,y as yt,ae as Nze,aR as Zl,S as Rze,al as cn,aS as Ca,af as ba,ag as wh,p as pe$1,V as Vu,aT as Hn,q as qr,X,aU as Xa,ap as y$1,aq as f,Z as Mt,aV as pv,ao as fv,N as Nd}from'./main.js';import {e}from'./chunk-CLNpRTgV.js';import {y,p,g,d}from'./chunk-o02BdfRM.js';var ne=`[
  {
    "version": "v0.9",
    "createSurface": {
...
`;var R="updateComponents",U="components",fe="registerMockRules",ge="mockRulesConfig",ye="rules",ve="id",Se="children",re="mock_rules_container",w=class i{destroyRef=_(Bn);chatState=_(y);catalogManagement=_(se$1);startupConfigState=_(cB);previousCatalogId=null;isDraftModified=false;_activeDraft=yt("");activeDraft=this._activeDraft.asReadonly();_draftInput=yt("");constructor(){Nze(this.startupConfigState.selectedRendererId).pipe(Zl(1),Rze(this.destroyRef)).subscribe(()=>{this.flushDraft();}),Nze(this.catalogManagement.activeCatalog).pipe(cn(t=>!!t),Rze(this.destroyRef)).subscribe(t=>{let n=t.catalogId||t.$id||"",a=this.previousCatalogId===null,r=this.previousCatalogId!==null&&this.previousCatalogId!==n;if((a||r)&&!this.isDraftModified){let o=this.getInitialDraft(n);this._activeDraft.set(o),this._draftInput.set(o);}this.previousCatalogId=n;}),(this.startupConfigState.sharedA2uiPayload?Nze(this.startupConfigState.sharedA2uiPayload):Ca(null)).pipe(cn(t=>!!t),Rze(this.destroyRef)).subscribe(t=>{this.injectExternalDraft(t);}),Nze(this._draftInput).pipe(Zl(1),ba(300),wh(),Rze(this.destroyRef)).subscribe(t=>{this.syncLayoutToHistory(t);});}updateDraft(e){this.isDraftModified=true,this._activeDraft.set(e),this._draftInput.set(e);}injectExternalDraft(e){this.isDraftModified=true,this._activeDraft.set(e),this._draftInput.set(e);}hydrateActiveDraft(){return this._activeDraft()}commitLayoutFromLlm(e){this.isDraftModified=true,this._activeDraft.set(e);}flushDraft(){this.isDraftModified=false,this.previousCatalogId=null;let e=this.catalogManagement.activeCatalog(),t=e&&(e.catalogId||e.$id)||"";!this.startupConfigState.activeRenderer()?.samplePayload&&!t&&(t="https://a2ui.org/specification/v0_9/basic_catalog.json");let a=this.getInitialDraft(t);this._activeDraft.set(a),this._draftInput.set(a);}getInitialDraft(e){let t=this.startupConfigState.activeRenderer();return t?.samplePayload?t.samplePayload:e==="https://a2ui.org/specification/v0_9/basic_catalog.json"?ne:e?p([{version:"v0.9",createSurface:{surfaceId:"sample-surface",catalogId:e,sendDataModel:true}}]):""}syncLayoutToHistory(e){let t=this.sanitizeLayout(e);if(!t)return;let n=this.chatState.chatHistory();if(n.length===0){this.chatState.setChatHistory([{role:"user",content:t}]);return}let a=n[n.length-1];if(a.role==="user"&&a.content.trim().startsWith("[")){let o=[...n];o[o.length-1]={role:"user",content:t},this.chatState.setChatHistory(o);}else this.chatState.updateChatHistory(o=>[...o,{role:"user",content:t}]);}sanitizeLayout(e){let t=e.trim();if(!t)return "";let n=g(t);if(n.success){let a=n.data.map(r=>r&&typeof r=="object"&&!Array.isArray(r)?this.sanitizeBlock(r):r).filter(r=>r!==null);return p(a)}return console.warn("[StateSync] Discarding malformed layout JSON during sanitization: not a valid JSON array"),""}sanitizeBlock(e){if(e[fe]||e[ge])return null;if(e[R]&&typeof e[R]=="object"&&e[R]!==null){let t=e[R];if(Array.isArray(t[U])){let n=t[U].filter(a=>a!==null&&typeof a=="object"&&!Array.isArray(a)?a[ve]!==re:true);t[U]=n.map(a=>a!==null&&typeof a=="object"&&!Array.isArray(a)?this.sanitizeComponentObject(a):a);}}return e}sanitizeComponentObject(e){let t={};for(let[n,a]of Object.entries(e))n===ye||/^mock/i.test(n)||(n===Se&&Array.isArray(a)?t[n]=a.filter(r=>r!==re):a!==null&&typeof a=="object"&&!Array.isArray(a)?t[n]=this.sanitizeComponentObject(a):Array.isArray(a)?t[n]=a.map(r=>r!==null&&typeof r=="object"&&!Array.isArray(r)?this.sanitizeComponentObject(r):r):t[n]=a);return t}static \u0275fac=function(t){return new(t||i)};static \u0275prov=pe$1({token:i,factory:i.\u0275fac,providedIn:"root"})};var oe=/```(?:jsonl?|jsonlines|a2ui|html|xml)?\s*([\s\S]*?)\s*```/gi,ie=/<(thought|thinking|reasoning)>([\s\S]*?)(?:<\/\1>|$)/gi,se=/\s*●●●\s*$/g;function ce(i){return i.length>0&&i.some(e=>{if(e&&typeof e=="object"&&!Array.isArray(e)){let t=Object.keys(e);return t.includes("version")||t.includes("createSurface")||t.includes("updateComponents")||t.includes("updateDataModel")||t.includes("deleteSurface")}return  false})}var E=class i{PULSE_INDICATOR="\u25CF\u25CF\u25CF";appendPulse(e){return `${e??""} ${this.PULSE_INDICATOR}`}stripPulse(e){return e?(se.lastIndex=0,e.replace(se,"").trim()):""}stripThinkingTags(e){return e?(ie.lastIndex=0,e.replace(ie,"").trim()):""}extractCodeFences(e){if(!e)return {extracted:"",hasFences:false};oe.lastIndex=0;let t=Array.from(e.matchAll(oe));return t.length>0?{extracted:t.map(n=>n[1].trim()).join(`
`),hasFences:true}:{extracted:e.trim(),hasFences:false}}cleanPayload(e){if(!e)return "";let t=this.stripPulse(e);if(t=this.stripThinkingTags(t),t=this.extractCodeFences(t).extracted,!t.startsWith("{")&&!t.startsWith("[")){let a=Array.from(t.matchAll(/[\{\[]/g));for(let r of a)if(r.index!==void 0&&r.index>=0){let o=t.substring(r.index).trim();if(o.startsWith("{")&&o.includes('"version"')||o.startsWith("[")&&/^\[\s*[\{\"]/.test(o)&&(o.includes('"version"')||o.includes('"createSurface"')||o.includes('"updateComponents"'))){t=o;break}let l=g(o);if(l.success&&ce(l.data)){t=o;break}}}return t.trim()}isLayoutSnapshot(e){if(!e)return  false;let t=this.cleanPayload(e);if(t.startsWith('{"version"')||t.startsWith("{")&&t.includes('"version"')||t.startsWith("[")&&(t.includes('"version"')||t.includes('"createSurface"')||t.includes('"updateComponents"')))return  true;let n=g(t);return n.success&&ce(n.data)}static \u0275fac=function(t){return new(t||i)};static \u0275prov=pe$1({token:i,factory:i.\u0275fac,providedIn:"root"})};function ue(i){if(i==null||i.trim().length===0)return {success:true,isConversational:true,blocks:[],count:0};let e=g(i);if(e.success)return {success:true,isConversational:false,blocks:e.data,count:F(e.data)};let t=i.trim();if(t.startsWith("{")||t.startsWith("[")){let s=le(i);if(s!==null&&typeof s=="object"){let c=Array.isArray(s)?s:[s];return {success:true,isConversational:false,blocks:c,count:F(c)}}}let a=i.split(`
`).map((s,c)=>({text:(s||"").trim(),originalIndex:c})).filter(s=>s.text.length>0),r=[],o=false,l=null;for(let s of a)if(!(s.text.startsWith("```")||!s.text.startsWith("{")&&!s.text.startsWith("["))){o=true;try{r.push(JSON.parse(s.text));}catch(c){let u=le(s.text);if(u!==null)r.push(u);else if(!l){let p=d(c,s.text);l={success:false,error:c?.message??"Syntax recovery failed",line:s.originalIndex+1,column:p.column,snippet:s.text};}}}if(r.length===0){if(l)return l;if(o){let s=e.error;return {success:false,error:s?.message??"Syntax recovery failed",line:s?.line,column:s?.column,snippet:s?.snippet}}return {success:true,isConversational:true,blocks:[],count:0}}return {success:true,isConversational:false,blocks:r,count:F(r)}}function le(i){if(i==null||i.trim().length===0)return null;let e=i.trim();if(e.length>256*1024)return null;e=e.replace(/,\s*([\]}])/g,"$1");try{return JSON.parse(e)}catch{for(let n=1;n<=5;n++)try{return JSON.parse(e+"}".repeat(n))}catch{}for(let n=1;n<=3;n++)for(let a=1;a<=3;a++)try{return JSON.parse(e+"}".repeat(n)+"]".repeat(a))}catch{}for(let n=1;n<=3;n++)for(let a=1;a<=3;a++)try{return JSON.parse(e+"]".repeat(n)+"}".repeat(a))}catch{}}return null}function be(i){if(!i||typeof i!="object")return  false;let e=i;if(!e.updateComponents||typeof e.updateComponents!="object")return  false;let t=e.updateComponents;return Array.isArray(t.components)}function pe(i,e){let t=false,n={};if(e)for(let r of Object.keys(e)){let o=r.toLowerCase().replace(/[^a-z]/g,"");n[o]=r;}let a={textbox:"textfield",textinput:"textfield",rowlayout:"row",columnlayout:"column",choice:"choicepicker",datepicker:"datetimeinput",datetimepicker:"datetimeinput"};for(let r of i){if(!be(r))continue;let o=r.updateComponents,l=[];for(let s of o.components){if(!s||typeof s!="object"||Array.isArray(s)){l.push(s);continue}let c=s,u=c.component;if(c.name&&!c.component&&(t=true,u=c.name,c.component=u,delete c.name),typeof u!="string")throw new Error("Component declaration is missing component type name string.");let p=u;if(e&&!e[u]){let d=u.toLowerCase().replace(/[^a-z]/g,""),g=n[d];if(!g){let f=a[d];f&&(g=n[f]);}if(g&&e[g])t=true,p=g;else {let f=d?Object.keys(e).find(I=>I.toLowerCase().includes(d)||d.includes(I.toLowerCase())):void 0;if(f)t=true,p=f;else throw new Error(`Validation failure: Component type "${u}" is not registered in the active custom catalog.`)}}let h=Ce(c);h.component=p,l.push(h);}o.components=l;}return t}function z(i){if(i===null||typeof i!="object")return i;if(Array.isArray(i))return i.map(n=>z(n));let e=i,t={};for(let[n,a]of Object.entries(e))n==="__proto__"||n==="constructor"||n==="prototype"||(t[n]=z(a));return t}function Ce(i){return z(i)}function F(i){return i.reduce((e,t)=>!t||typeof t!="object"?e:t.updateComponents&&Array.isArray(t.updateComponents.components)?e+t.updateComponents.components.length:t.createSurface?e+1:e,0)}var P=class i{catalogManagement=_(se$1);mcpManager=_(fv);systemPrompt=Nd(()=>{let e=this.catalogManagement.activeCatalog(),n=this.mcpManager.doesCatalogSupportMcp(e)?this.mcpManager.getActiveServersWithTools():[],a=this.buildMcpInstructions(n);return e?this.generateSystemPrompt(p(e))+a:`
  ...
  ${a}
      `});buildMcpInstructions(e){let t=new Map;for(let r of e)for(let o of r.tools||[])t.has(o.name)||t.set(o.name,o);if(t.size===0)return "";let n={type:"object",properties:{content:{type:"array",items:{type:"object",properties:{type:{type:"string"},text:{type:"string"}}}}}};return `
  ...
  ### Available MCP Tools
  ${Array.from(t.values()).map(r=>{let o=r.description?` - ${r.description}`:"",l=JSON.stringify(r.inputSchema||{type:"object",properties:{}}),s=JSON.stringify(r.outputSchema||n);return `- **\`${r.name}\`**${o}
  - **Input Schema**: \`${l}\`
