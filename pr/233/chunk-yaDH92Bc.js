import {_,a7 as Bn,s as se$1,b as sB,y as yt,ae as Nze,aT as Zl,U as Rze,al as cn,aU as Ca,af as ba,ag as Eh,p as pe$1,V as Vu,aV as Hn,q as qr,X,aW as Xa,ap as y$1,aq as f,Z as Mt,aX as pv,ao as fv,$ as $s,N as Nd}from'./main.js';import {e}from'./chunk-CLNpRTgV.js';import {y,p,g,d}from'./chunk-o02BdfRM.js';var re=`[
  {
    "version": "v0.9",
    "createSurface": {
...
`;var P="updateComponents",F="components",ye="registerMockRules",ve="mockRulesConfig",Se="rules",Ie="id",Ce="children",oe="mock_rules_container",w=class o{destroyRef=_(Bn);chatState=_(y);catalogManagement=_(se$1);startupConfigState=_(sB);previousCatalogId=null;isDraftModified=false;_activeDraft=yt("");activeDraft=this._activeDraft.asReadonly();_draftInput=yt("");constructor(){Nze(this.startupConfigState.selectedRendererId).pipe(Zl(1),Rze(this.destroyRef)).subscribe(()=>{this.flushDraft();}),Nze(this.catalogManagement.activeCatalog).pipe(cn(t=>!!t),Rze(this.destroyRef)).subscribe(t=>{let n=t.catalogId||t.$id||"",a=this.previousCatalogId===null,r=this.previousCatalogId!==null&&this.previousCatalogId!==n;if((a||r)&&!this.isDraftModified){let i=this.getInitialDraft(n);this._activeDraft.set(i),this._draftInput.set(i);}this.previousCatalogId=n;}),(this.startupConfigState.sharedA2uiPayload?Nze(this.startupConfigState.sharedA2uiPayload):Ca(null)).pipe(cn(t=>!!t),Rze(this.destroyRef)).subscribe(t=>{this.injectExternalDraft(t);}),Nze(this._draftInput).pipe(Zl(1),ba(300),Eh(),Rze(this.destroyRef)).subscribe(t=>{this.syncLayoutToHistory(t);});}updateDraft(e){this.isDraftModified=true,this._activeDraft.set(e),this._draftInput.set(e);}injectExternalDraft(e){this.isDraftModified=true,this._activeDraft.set(e),this._draftInput.set(e);}hydrateActiveDraft(){return this._activeDraft()}commitLayoutFromLlm(e){this.isDraftModified=true,this._activeDraft.set(e);}flushDraft(){this.isDraftModified=false,this.previousCatalogId=null;let e=this.catalogManagement.activeCatalog(),t=e&&(e.catalogId||e.$id)||"";!this.startupConfigState.activeRenderer()?.samplePayload&&!t&&(t="https://a2ui.org/specification/v0_9/basic_catalog.json");let a=this.getInitialDraft(t);this._activeDraft.set(a),this._draftInput.set(a);}getInitialDraft(e){let t=this.startupConfigState.activeRenderer();return t?.samplePayload?t.samplePayload:e==="https://a2ui.org/specification/v0_9/basic_catalog.json"?re:e?p([{version:"v0.9",createSurface:{surfaceId:"sample-surface",catalogId:e,sendDataModel:true}}]):""}syncLayoutToHistory(e){let t=this.sanitizeLayout(e);if(!t)return;let n=this.chatState.chatHistory();if(n.length===0){this.chatState.setChatHistory([{role:"user",content:t}]);return}let a=n[n.length-1];if(a.role==="user"&&a.content.trim().startsWith("[")){let i=[...n];i[i.length-1]={role:"user",content:t},this.chatState.setChatHistory(i);}else this.chatState.updateChatHistory(i=>[...i,{role:"user",content:t}]);}sanitizeLayout(e){let t=e.trim();if(!t)return "";let n=g(t);if(n.success){let a=n.data.map(r=>r&&typeof r=="object"&&!Array.isArray(r)?this.sanitizeBlock(r):r).filter(r=>r!==null);return p(a)}return console.warn("[StateSync] Discarding malformed layout JSON during sanitization: not a valid JSON array"),""}sanitizeBlock(e){if(e[ye]||e[ve])return null;if(e[P]&&typeof e[P]=="object"&&e[P]!==null){let t=e[P];if(Array.isArray(t[F])){let n=t[F].filter(a=>a!==null&&typeof a=="object"&&!Array.isArray(a)?a[Ie]!==oe:true);t[F]=n.map(a=>a!==null&&typeof a=="object"&&!Array.isArray(a)?this.sanitizeComponentObject(a):a);}}return e}sanitizeComponentObject(e){let t={};for(let[n,a]of Object.entries(e))n===Se||/^mock/i.test(n)||(n===Ce&&Array.isArray(a)?t[n]=a.filter(r=>r!==oe):a!==null&&typeof a=="object"&&!Array.isArray(a)?t[n]=this.sanitizeComponentObject(a):Array.isArray(a)?t[n]=a.map(r=>r!==null&&typeof r=="object"&&!Array.isArray(r)?this.sanitizeComponentObject(r):r):t[n]=a);return t}static \u0275fac=function(t){return new(t||o)};static \u0275prov=pe$1({token:o,factory:o.\u0275fac,providedIn:"root"})};var ie=/```(?:jsonl?|jsonlines|a2ui|html|xml)?\s*([\s\S]*?)\s*```/gi,se=/<(thought|thinking|reasoning)>([\s\S]*?)(?:<\/\1>|$)/gi,ce=/\s*●●●\s*$/g;function le(o){return o.length>0&&o.some(e=>{if(e&&typeof e=="object"&&!Array.isArray(e)){let t=Object.keys(e);return t.includes("version")||t.includes("createSurface")||t.includes("updateComponents")||t.includes("updateDataModel")||t.includes("deleteSurface")}return  false})}var E=class o{PULSE_INDICATOR="\u25CF\u25CF\u25CF";appendPulse(e){return `${e??""} ${this.PULSE_INDICATOR}`}stripPulse(e){return e?(ce.lastIndex=0,e.replace(ce,"").trim()):""}stripThinkingTags(e){return e?(se.lastIndex=0,e.replace(se,"").trim()):""}extractCodeFences(e){if(!e)return {extracted:"",hasFences:false};ie.lastIndex=0;let t=Array.from(e.matchAll(ie));return t.length>0?{extracted:t.map(n=>n[1].trim()).join(`
`),hasFences:true}:{extracted:e.trim(),hasFences:false}}cleanPayload(e){if(!e)return "";let t=this.stripPulse(e);if(t=this.stripThinkingTags(t),t=this.extractCodeFences(t).extracted,!t.startsWith("{")&&!t.startsWith("[")){let a=Array.from(t.matchAll(/[\{\[]/g));for(let r of a)if(r.index!==void 0&&r.index>=0){let i=t.substring(r.index).trim();if(i.startsWith("{")&&i.includes('"version"')||i.startsWith("[")&&/^\[\s*[\{\"]/.test(i)&&(i.includes('"version"')||i.includes('"createSurface"')||i.includes('"updateComponents"'))){t=i;break}let l=g(i);if(l.success&&le(l.data)){t=i;break}}}return t.trim()}isLayoutSnapshot(e){if(!e)return  false;let t=this.cleanPayload(e);if(t.startsWith('{"version"')||t.startsWith("{")&&t.includes('"version"')||t.startsWith("[")&&(t.includes('"version"')||t.includes('"createSurface"')||t.includes('"updateComponents"')))return  true;let n=g(t);return n.success&&le(n.data)}static \u0275fac=function(t){return new(t||o)};static \u0275prov=pe$1({token:o,factory:o.\u0275fac,providedIn:"root"})};function de(o){if(o==null||o.trim().length===0)return {success:true,isConversational:true,blocks:[],count:0};let e=g(o);if(e.success)return {success:true,isConversational:false,blocks:e.data,count:z(e.data)};let t=o.trim();if(t.startsWith("{")||t.startsWith("[")){let s=ue(o);if(s!==null&&typeof s=="object"){let c=Array.isArray(s)?s:[s];return {success:true,isConversational:false,blocks:c,count:z(c)}}}let a=o.split(`
`).map((s,c)=>({text:(s||"").trim(),originalIndex:c})).filter(s=>s.text.length>0),r=[],i=false,l=null;for(let s of a)if(!(s.text.startsWith("```")||!s.text.startsWith("{")&&!s.text.startsWith("["))){i=true;try{r.push(JSON.parse(s.text));}catch(c){let u=ue(s.text);if(u!==null)r.push(u);else if(!l){let d$1=d(c,s.text);l={success:false,error:c?.message??"Syntax recovery failed",line:s.originalIndex+1,column:d$1.column,snippet:s.text};}}}if(r.length===0){if(l)return l;if(i){let s=e.error;return {success:false,error:s?.message??"Syntax recovery failed",line:s?.line,column:s?.column,snippet:s?.snippet}}return {success:true,isConversational:true,blocks:[],count:0}}return {success:true,isConversational:false,blocks:r,count:z(r)}}function ue(o){if(o==null||o.trim().length===0)return null;let e=o.trim();if(e.length>256*1024)return null;e=e.replace(/,\s*([\]}])/g,"$1");try{return JSON.parse(e)}catch{for(let n=1;n<=5;n++)try{return JSON.parse(e+"}".repeat(n))}catch{}for(let n=1;n<=3;n++)for(let a=1;a<=3;a++)try{return JSON.parse(e+"}".repeat(n)+"]".repeat(a))}catch{}for(let n=1;n<=3;n++)for(let a=1;a<=3;a++)try{return JSON.parse(e+"]".repeat(n)+"}".repeat(a))}catch{}}return null}function be(o){if(!o||typeof o!="object")return  false;let e=o;if(!e.updateComponents||typeof e.updateComponents!="object")return  false;let t=e.updateComponents;return Array.isArray(t.components)}function pe(o,e){let t=false,n={};if(e)for(let r of Object.keys(e)){let i=r.toLowerCase().replace(/[^a-z]/g,"");n[i]=r;}let a={textbox:"textfield",textinput:"textfield",rowlayout:"row",columnlayout:"column",choice:"choicepicker",datepicker:"datetimeinput",datetimepicker:"datetimeinput"};for(let r of o){if(!be(r))continue;let i=r.updateComponents,l=[];for(let s of i.components){if(!s||typeof s!="object"||Array.isArray(s)){l.push(s);continue}let c=s,u=c.component;if(c.name&&!c.component&&(t=true,u=c.name,c.component=u,delete c.name),typeof u!="string")throw new Error("Component declaration is missing component type name string.");let d=u;if(e&&!e[u]){let p=u.toLowerCase().replace(/[^a-z]/g,""),g=n[p];if(!g){let f=a[p];f&&(g=n[f]);}if(g&&e[g])t=true,d=g;else {let f=p?Object.keys(e).find(k=>k.toLowerCase().includes(p)||p.includes(k.toLowerCase())):void 0;if(f)t=true,d=f;else throw new Error(`Validation failure: Component type "${u}" is not registered in the active custom catalog.`)}}let h=xe(c);h.component=d,l.push(h);}i.components=l;}return t}function J(o){if(o===null||typeof o!="object")return o;if(Array.isArray(o))return o.map(n=>J(n));let e=o,t={};for(let[n,a]of Object.entries(e))n==="__proto__"||n==="constructor"||n==="prototype"||(t[n]=J(a));return t}function xe(o){return J(o)}function z(o){return o.reduce((e,t)=>!t||typeof t!="object"?e:t.updateComponents&&Array.isArray(t.updateComponents.components)?e+t.updateComponents.components.length:t.createSurface?e+1:e,0)}function me(o){return o?!!(o.functions&&"callMcpTool"in o.functions||o.$defs&&("callMcpTool"in o.$defs||"catalog_callMcpTool"in o.$defs)):false}var O=class o{catalogManagement=_(se$1);mcpManager=_(fv);localStorageInteractions=_($s);customInstructionsState=yt(this.loadInitialState());presets=Nd(()=>this.customInstructionsState().presets);activePresetId=Nd(()=>this.customInstructionsState().activePresetId);activePreset=Nd(()=>{let e=this.customInstructionsState();return e.presets.find(t=>t.id===e.activePresetId)??null});customInstructions=Nd(()=>this.activePreset()?.content??"");hasCustomInstructions=Nd(()=>this.customInstructions().trim().length>0);systemPrompt=Nd(()=>{let e=this.catalogManagement.activeCatalog(),n=me(e)?this.mcpManager.getActiveServersWithTools():[],a=this.buildMcpInstructions(n),r=this.customInstructions().trim(),i=r?`

## Custom User Instructions

${r}`:"";return e?(this.generateSystemPrompt(p(e))+a).trimEnd()+i:`
  ...
  ${a}
      `.trimEnd()+i});setCustomInstructionsState(e){let t=e.presets.some(a=>a.id===e.activePresetId),n={presets:e.presets,activePresetId:t?e.activePresetId:null};this.customInstructionsState.set(n),this.localStorageInteractions.setItem("a2ui_composer_custom_instructions",JSON.stringify(n));}loadInitialState(){let e=this.localStorageInteractions.getItem("a2ui_composer_custom_instructions");if(!e)return {presets:[],activePresetId:null};try{let t=JSON.parse(e);if(!t||!Array.isArray(t.presets))return {presets:[],activePresetId:null};let n=t.presets.filter(r=>typeof r=="object"&&r!==null&&typeof r.id=="string"&&typeof r.name=="string"&&typeof r.content=="string"),a=typeof t.activePresetId=="string"&&n.some(r=>r.id===t.activePresetId)?t.activePresetId:null;return {presets:n,activePresetId:a}}catch{return {presets:[],activePresetId:null}}}buildMcpInstructions(e){let t=new Map;for(let r of e)for(let i of r.tools||[])t.has(i.name)||t.set(i.name,i);if(t.size===0)return "";let n={type:"object",properties:{content:{type:"array",items:{type:"object",properties:{type:{type:"string"},text:{type:"string"}}}}}};return `
