...
`;var j="updateComponents",oe="components",At="registerMockRules",xt="mockRulesConfig",wt="rules",Tt="id",Dt="children",ot="mock_rules_container",U=class r{destroyRef=f(ce);chatState=f(o);catalogManagement=f(eE);startupResolution=f(Ao,{optional:true});_activeDraft=B("");activeDraft=this._activeDraft.asReadonly();_draftInput=B("");constructor(){let a=this.startupResolution?.selectedRendererId$?aY(this.startupResolution.selectedRendererId$):Ee(null),e=aY(this.catalogManagement.activeCatalog),t=this.startupResolution?.sharedA2uiPayload?aY(this.startupResolution.sharedA2uiPayload):Ee(null);kl(a,e).pipe(H_(this.destroyRef)).subscribe(()=>{let n=this.catalogManagement.activeCatalog(),i=n&&(n.catalogId||n.$id)||"",o=this._activeDraft(),s=this.getCatalogIdFromDraft(o);if(o===""||s!==i){let l=this.getInitialDraft(i);this._activeDraft.set(l),this._draftInput.set(l);}}),t.pipe(ue(n=>!!n),H_(this.destroyRef)).subscribe(n=>{this.updateDraft(n);}),aY(this._draftInput).pipe(Wo(1),Yn(300),Ys(),H_(this.destroyRef)).subscribe(n=>{this.syncLayoutToHistory(n);});}updateDraft(a){this._activeDraft.set(a),this._draftInput.set(a);}hydrateActiveDraft(){return this._activeDraft()}commitLayoutFromLlm(a){this._activeDraft.set(a);}flushDraft(){let a=this.catalogManagement.activeCatalog(),e=a&&(a.catalogId||a.$id)||"";this._activeDraft.set(this.getInitialDraft(e));}getInitialDraft(a){let e=this.startupResolution?.sharedA2uiPayload();if(e)return e;let t=this.startupResolution?.activeRenderer();return t?.samplePayload?t.samplePayload:a==="https://a2ui.org/specification/v0_9/basic_catalog.json"?rt:a?p([{version:"v0.9",createSurface:{surfaceId:"sample-surface",catalogId:a,sendDataModel:true}}]):""}getCatalogIdFromDraft(a){let e=a.trim();if(!e)return null;try{let t=JSON.parse(e);if(Array.isArray(t))for(let n of t){let i=n;if(i?.createSurface?.catalogId)return i.createSurface.catalogId}else if(t&&typeof t=="object"){let n=t;if(n?.createSurface?.catalogId)return n.createSurface.catalogId}}catch{}return null}syncLayoutToHistory(a){let e=this.sanitizeLayout(a);if(!e)return;let t=this.chatState.chatHistory();if(t.length===0){this.chatState.setChatHistory([{role:"user",content:e}]);return}let n=t[t.length-1];if(n.role==="user"&&n.content.trim().startsWith("[")){let o=[...t];o[o.length-1]={role:"user",content:e},this.chatState.setChatHistory(o);}else this.chatState.updateChatHistory(o=>[...o,{role:"user",content:e}]);}sanitizeLayout(a){let e=a.trim();if(!e)return "";let t=l(e);if(t){let n=t.map(i=>i&&typeof i=="object"&&!Array.isArray(i)?this.sanitizeBlock(i):i).filter(i=>i!==null);return p(n)}return console.warn("[StateSync] Discarding malformed layout JSON during sanitization: not a valid JSON array"),""}sanitizeBlock(a){if(a[At]||a[xt])return null;if(a[j]&&typeof a[j]=="object"&&a[j]!==null){let e=a[j];if(Array.isArray(e[oe])){let t=e[oe].filter(n=>n!==null&&typeof n=="object"&&!Array.isArray(n)?n[Tt]!==ot:true);e[oe]=t.map(n=>n!==null&&typeof n=="object"&&!Array.isArray(n)?this.sanitizeComponentObject(n):n);}}return a}sanitizeComponentObject(a){let e={};for(let[t,n]of Object.entries(a))t===wt||/^mock/i.test(t)||(t===Dt&&Array.isArray(n)?e[t]=n.filter(i=>i!==ot):n!==null&&typeof n=="object"&&!Array.isArray(n)?e[t]=this.sanitizeComponentObject(n):Array.isArray(n)?e[t]=n.map(i=>i!==null&&typeof i=="object"&&!Array.isArray(i)?this.sanitizeComponentObject(i):i):e[t]=n);return e}static \u0275fac=function(e){return new(e||r)};static \u0275prov=w({token:r,factory:r.\u0275fac,providedIn:"root"})};var st=/```(?:jsonl?|jsonlines|a2ui|html|xml)?\s*([\s\S]*?)\s*```/gi,ct=/<(thought|thinking|reasoning)>([\s\S]*?)(?:<\/\1>|$)/gi,lt=/\s*●●●\s*$/g,F=class r{PULSE_INDICATOR="\u25CF\u25CF\u25CF";appendPulse(a){return `${a??""} ${this.PULSE_INDICATOR}`}stripPulse(a){return a?(lt.lastIndex=0,a.replace(lt,"").trim()):""}stripThinkingTags(a){return a?(ct.lastIndex=0,a.replace(ct,"").trim()):""}extractCodeFences(a){if(!a)return {extracted:"",hasFences:false};st.lastIndex=0;let e=Array.from(a.matchAll(st));return e.length>0?{extracted:e.map(t=>t[1].trim()).join(`
...
`).map(i=>i.trim()).filter(i=>i.length>0),n=[];for(let i of t)if(!(i.startsWith("```")||!i.startsWith("{")&&!i.startsWith("[")))try{n.push(JSON.parse(i));}catch{a=true;let s=Rt(i);if(s!==null)n.push(s);else if(i.includes('"version"')||i.includes('"createSurface"'))throw new Error(`Syntax recovery failed for corrupted JSON Line:
"${i}"`)}if(n.length===0)throw new Error("No valid A2UI JSON layout command block could be parsed or recovered.");return {blocks:n,wasHealed:a}}function Rt(r){if(r==null||r.trim().length===0)return null;let a=r.trim();a=a.replace(/,\s*([\]}])/g,"$1");try{return JSON.parse(a)}catch{for(let t=1;t<=5;t++)try{return JSON.parse(a+"}".repeat(t))}catch{}for(let t=1;t<=3;t++)for(let n=1;n<=3;n++)try{return JSON.parse(a+"]".repeat(t)+"}".repeat(n))}catch{}}return null}function Mt(r){if(!r||typeof r!="object")return  false;let a=r;if(!a.updateComponents||typeof a.updateComponents!="object")return  false;let e=a.updateComponents;return Array.isArray(e.components)}function mt(r,a){let e=false,t={};if(a)for(let i of Object.keys(a)){let o=i.toLowerCase().replace(/[^a-z]/g,"");t[o]=i;}let n={textbox:"textfield",textinput:"textfield",rowlayout:"row",columnlayout:"column",choice:"choicepicker",datepicker:"datetimeinput",datetimepicker:"datetimeinput"};for(let i of r){if(!Mt(i))continue;let o=i.updateComponents,s=[];for(let l of o.components){if(!l||typeof l!="object"||Array.isArray(l)){s.push(l);continue}let d=l,c=d.component;if(d.name&&!d.component&&(e=true,c=d.name,d.component=c,delete d.name),typeof c!="string")throw new Error("Component declaration is missing component type name string.");let p=c;if(a&&!a[c]){let u=c.toLowerCase().replace(/[^a-z]/g,""),g=t[u];if(!g){let f=n[u];f&&(g=t[f]);}if(g&&a[g])e=true,p=g;else {let f=u?Object.keys(a).find(I=>I.toLowerCase().includes(u)||u.includes(I.toLowerCase())):void 0;if(f)e=true,p=f;else throw new Error(`Validation failure: Component type "${c}" is not registered in the active custom catalog.`)}}let h=Et(d);h.component=p,s.push(h);}o.components=s;}return e}function se(r){if(r===null||typeof r!="object")return r;if(Array.isArray(r))return r.map(t=>se(t));let a=r,e={};for(let[t,n]of Object.entries(a))e[t]=se(n);return e}function Et(r){return se(r)}var H=class r{catalogManagement=f(eE);systemPrompt=vr(()=>{let a=this.catalogManagement.activeCatalog();return a?this.generateSystemPrompt(p(a)):`
