...
export function useA2uiSandbox<C extends ComponentApi = ComponentApi>(
  ...
  const [contentRenderRequest, setContentRenderRequest] = useState(0);
  const contentRenderWaiters = useRef<Array<() => void>>([]);

  useLayoutEffect(() => {
    for (const resolve of contentRenderWaiters.current.splice(0)) resolve();
    ...
    const connection = a2uiBridge.attachRenderer(processor, {
      ...
      whenSurfaceRendered: () =>
        new Promise<void>(resolve => {
          ...
          // and the surface object itself has not changed.
          setContentRenderRequest(request => request + 1);
        }),
      onSurfaceCleared: () => {
