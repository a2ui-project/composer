...
export function createSlackPreviewSession(
  ...
  const unsubscribeFromLowerSession = session.subscribe(() => {
    ...
    if (processingMessages) syncActiveSurfaceIds(session, activeSurfaceIds);
...
function addRecreateDeletes(
  ...
  for (const message of messages) {
    ...
    if (surfaceId) nextActiveSurfaceIds.add(surfaceId);

    const deletedSurfaceId = deleteSurfaceId(message);
    if (deletedSurfaceId) nextActiveSurfaceIds.delete(deletedSurfaceId);
  }
