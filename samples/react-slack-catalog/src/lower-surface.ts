...
export function createSlackPreviewSession(
  ...
  const subscribeToSurface = (surface: SurfaceModel) => {
    ...
    const subscription = surface.dataModel.subscribe('/', () => {
      if (disposed || processingBatch) return;
      rerender();
    });
  ...
  const subscribeToProcessorLifecycle = () => {
    lifecycleSubscriptions = [
      processor.onSurfaceCreated(surface => {
        ...
        for (const listener of surfaceCreatedListeners) listener(surface);
      }),
      processor.onSurfaceDeleted(surfaceId => {
        if (activeSurfaceSubscription?.surfaceId !== surfaceId) return;
        clearActiveSurfaceSubscription();
        const surface = lastSurface(processor);
        if (surface) subscribeToSurface(surface);
      }),
    ];
  };
  ...
  const session: SlackLoweringPreviewSession = {
    ...
    async dispatch(actionId) {
      ...
      const surface = lastSurface(processor);
      if (!isCurrentActionEntry(entry, surface)) return;
    ...
    updateInput(actionId, value) {
      ...
      if (!entry || entry.revision !== revision || entry.surface !== lastSurface(processor)) return;
      try {
...
function collectActionIds(value: unknown, actionIds = new Set<string>()): Set<string> {
  ...
  if (!isRecord(value)) return actionIds;

  if (typeof value.action_id === 'string') actionIds.add(value.action_id);
  for (const child of Object.values(value)) collectActionIds(child, actionIds);
  return actionIds;
}
...
function validateSlackOutput(blocks: readonly KnownBlock[]): void {
  ...
  for (const block of blocks) {
    ...
    if (blockType === 'actions') {
      ...
      for (const element of elements) validateActionElement(element);
    }
...
function validateActionElement(element: unknown): void {
  if (stringField(element, 'type') !== 'button') return;
...
function assertTextLimit(text: string | undefined, limit: number, code: string): void {
  if (text === undefined || text.length <= limit) return;
  throw new LoweringError(
    code,
...
function collectOutputLimitDiagnostics(
  ...
  for (const code of evidence.truncatedCodes) pushTruncationDiagnostic(diagnostics, code);
}

function pushDiagnostic(diagnostics: PreviewDiagnostic[], diagnostic: PreviewDiagnostic): void {
  if (diagnostics.some(({code}) => code === diagnostic.code)) return;
...
function collectOutputLimitEvidence(nodes: readonly ChannelNode[]): OutputLimitEvidence {
  ...
  const visit = (node: ChannelNode): boolean => {
    ...
    if (nodeType === 'actions') {
      ...
      if (children.length > SLACK_LIMITS.actionsElements) actionsElementsTrimmed = true;
      for (const child of children) actionsElementsTrimmed = visit(child) || actionsElementsTrimmed;
  ...
  let actionsElementsTrimmed = false;
  for (const node of nodes) actionsElementsTrimmed = visit(node) || actionsElementsTrimmed;
  return {
...
function addTruncationEvidence(
  ...
  if (text.length > limit) truncatedCodes.add(code);
}
...
function channelChildNodes(node: ChannelNode): ChannelNode[] {
  ...
  if (Array.isArray(children)) return children.filter(isChannelNode);
...
function lowerComponent(
  ...
  if (!model) throw new WaitingForComponentError(componentId);
...
function lowerText(
  ...
  if (variant === 'caption') return Context({children: text});
...
function collectText(nodes: ChannelNode[]): string {
  return nodes
    .flatMap(node => {
      ...
      if (node.type === 'text' && typeof node.props.value === 'string') return [node.props.value];
