...
export function createEventActionDispatchEntry({
  ...
  if ('functionCall' in parsedAction) return undefined;
  return {
    actionId,
    revision,
    surface,
...
export async function dispatchCurrentEventAction(
  ...
  if (!entry || !state.surface) return false;
  if (entry.surface !== state.surface) return false;
  if (entry.surfaceId !== state.surface.id) return false;
