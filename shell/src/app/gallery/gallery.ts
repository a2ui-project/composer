...
export class Gallery implements OnInit, OnDestroy {
  ...
  /** The stringified full JSON schema for the selected component. */
  protected readonly selectedComponentSchema = computed<string>(() => {
    const key = this.selectedComponentKey();
    const catalog = this.catalogManagement.activeCatalog();
    const comp = key ? catalog?.components?.[key] : null;
    return comp ? JSON.stringify(comp, null, 2) : '';
  });
