...
/**
 * Harness for interacting with the RendererSelectorComponent in tests.
 */
export class RendererSelectorHarness extends ComponentHarness {
  ...
  async getOptionsText(): Promise<string[]> {
    ...
    const getLabels = this.locatorFactory
      .documentRootLocatorFactory()
      .locatorForAll('.renderer-option-label');
    const labels = await getLabels();
    if (labels.length > 0) {
      return Promise.all(labels.map(async label => (await label.text()).trim()));
    }
    const getEmpty = this.locatorFactory
      .documentRootLocatorFactory()
      .locatorForAll('.empty-state-option');
    const emptyEls = await getEmpty();
    return Promise.all(emptyEls.map(async el => (await el.text()).trim()));
  }

  async getOptionSubtexts(): Promise<string[]> {
    await this.openSelect();
  ...
  async getDeleteButtonsForOptions(): Promise<MatButtonHarness[]> {
    ...
    const getButtons = this.locatorFactory
      .documentRootLocatorFactory()
      .locatorForAll(MatButtonHarness.with({selector: '.delete-renderer-button'}));
    return getButtons();
  }
...
/**
 * Harness for interacting with the AddRendererDialogComponent in tests.
 */
export class AddRendererDialogHarness extends ComponentHarness {
  static hostSelector = 'a2ui-composer-add-renderer-dialog';
