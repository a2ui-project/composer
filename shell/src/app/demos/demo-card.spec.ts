...
describe('DemoCard sandboxed live demo frame', () => {
  ...
  it('reveals committed renderer content immediately and ignores the startup screen', () => {
    vi.useFakeTimers();
    const fixture = mountCard(true);
    emitFromCard(fixture, PreviewBridgeMessageType.SURFACE_RESIZE, {height: 130, width: 480});
    emitFromCard(fixture, PreviewBridgeMessageType.SURFACE_RESIZE, {
      height: 166,
      width: 480,
      contentReady: false,
  ...
  it('fits overflowing content and requests a wide tile without reloading the frame', () => {
    vi.useFakeTimers();
    const fixture = mountCard(true);
    const frame = frameOf(fixture)!;
    vi.spyOn(frame, 'clientWidth', 'get').mockReturnValue(400);
    vi.spyOn(surfaceOf(fixture), 'clientWidth', 'get').mockReturnValue(400);

    emitFromCard(fixture, PreviewBridgeMessageType.SURFACE_RESIZE, {height: 500, width: 800});
    advance(fixture, MEASURE_SETTLE_MS);

    const host = fixture.nativeElement as HTMLElement;
    expect(host.style.getPropertyValue('--demo-preview-scale')).toBe('0.5');
    expect(fixture.componentInstance.needsWideLayout()).toBe(true);
    expect(surfaceOf(fixture).style.height).toBe('250px');
    expect(frameOf(fixture)).toBe(frame);

    // Widening the tile restores authored size without destroying live form state.
    vi.spyOn(surfaceOf(fixture), 'clientWidth', 'get').mockReturnValue(900);
    vi.spyOn(frame, 'clientWidth', 'get').mockReturnValue(900);
    emitFromCard(fixture, PreviewBridgeMessageType.SURFACE_RESIZE, {height: 500, width: 900});
    expect(host.style.getPropertyValue('--demo-preview-scale')).toBe('1');
    expect(frameOf(fixture)).toBe(frame);

    resolvedUrlSignal.set('http://localhost:3000/other-renderer');
    fixture.detectChanges();
    expect(fixture.componentInstance.needsWideLayout()).toBe(false);
  });

  it('compares content width with the viewport at measurement time', () => {
    const fixture = mountCard(true);
    const frame = frameOf(fixture)!;
    vi.spyOn(frame, 'clientWidth', 'get').mockReturnValue(1124);
    vi.spyOn(surfaceOf(fixture), 'clientWidth', 'get').mockReturnValue(1124);
    emitFromCard(fixture, PreviewBridgeMessageType.SURFACE_RESIZE, {
      height: 300,
      width: 1230,
      viewportWidth: 1230,
    ...
    expect(fixture.componentInstance.needsWideLayout()).toBe(false);
    expect(
      (fixture.nativeElement as HTMLElement).style.getPropertyValue('--demo-preview-scale'),
    ).toBe('1');
  });
