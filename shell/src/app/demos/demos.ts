...
export class Demos implements OnInit, OnDestroy {
  ...
  private readonly cards = viewChildren(DemoCard);

  /** Wide demos and their neighbours fill rows without dense packing or reordering. */
  ...
  private rankCandidates(): RankedCard[] {
    ...
    const candidates = new Map(this.visibleCards);
    // Resizing a bento row can briefly move an edited preview out of range.
    // Keep the focused frame alive so keyboard focus and unsaved form state survive.
    const focusedCard = document.activeElement?.closest('a2ui-composer-demo-card');
    if (focusedCard && this.wallScroller()?.nativeElement.contains(focusedCard)) {
      const focusedKey = readTrackKey(focusedCard);
      if (focusedKey) candidates.set(focusedKey, focusedCard);
    }
    for (const [trackKey, element] of candidates) {
      const rect = element.getBoundingClientRect();
      ranked.push({
        trackKey,
        focused: element === focusedCard,
...
interface RankedCard {
  ...
  /** A focused preview keeps one of the existing slots while its geometry changes. */
...
function canDisplace(held: RankedCard, challenger: RankedCard): boolean {
  if (held.focused !== challenger.focused) return challenger.focused;
