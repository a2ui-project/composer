    ...
    try {
      ...
      for (let pass = 0; pass < 2; pass++) {
        ...
        if (toolCalls.length) {
          ...
          if (this.isCancelRequested) throw this.cancelError();
          ...
          try {
            ...
            if (requestId === this.activeRequestId) this.expectedToolRendererUrl = undefined;
          }
          if (requestId !== this.activeRequestId) return;
          if (this.isCancelRequested) throw this.cancelError();
          rendererUrlAtSubmit = this.configProvider.rendererUrl();
          catalogIdAtSubmit = this.getActiveCatalogId();
          if (changedRenderer) this.stateSync.flushDraft();
      ...
      if (this.activeRequestId === requestId) this.toolAbortController = undefined;
