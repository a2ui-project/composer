 ...
 * a committed data module the Custom Catalog route can display verbatim.
 *
 * The catalog documented here is the shell's own native `@a2ui/angular`
 * catalog, so its source lives in this repo and is bundled from disk. That is
 * what keeps the Definitions and Renderers views from drifting: they are
 * generated from the same files the renderer actually runs, never transcribed.
 *
...
const outFile = join(catalogDir, 'catalog-source.generated.ts');
...
/** One bundled catalog source file, shown verbatim by the Custom Catalog route. */
